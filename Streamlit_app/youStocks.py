...
st.set_page_config(page_title="YouStock.AI",page_icon="chart_with_upwards_trend",layout="wide", initial_sidebar_state="auto")
...
today = date.today() 
st.write('''# YouStock.AI ''')
# Sidebar
# image_path = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\static\hack2skilllogo.jpg"

st.sidebar.image('hack2skilllogo.jpg', width=400, use_column_width="auto")
...
tickerDataset = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\datasets\TickersData.csv"
stock_df = pd.read_csv(tickerDataset)
#print(stock_df)
if(selected == 'Comparative Analysis of Stock Performances'):
    ...
    with st.spinner('Loading...'):
        time.sleep(2)
       # st.success('Loaded')
    dict_csv = pd.read_csv(tickerDataset, header=None, index_col=0).to_dict()[1]
    # print(dict_csv)
    symb_list = [] 
    for i in dropdown:  
        val = dict_csv.get(i)  
    ...
    with st.spinner('Loading...'):  # spinner while loading
        ...
        dict_csv = pd.read_csv(tickerDataset, header=None, index_col=0).to_dict()[1]  # read csv file
    ...
    st.write("Work by")
    st.subheader('[![Bhavya Pratap GitHub](https://img.shields.io/badge/Bhavya_Pratap-GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Ankur2606)') 
    st.subheader('[![Bhavya Madan GitHub](https://img.shields.io/badge/Bhavya_Madan-GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/bhavya1006)')
    ...
    #print(data.head())
        ...
        col1, col2 = st.columns([1, 1])
        nifty50Path = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\datasets\NIFTY 50.csv"
        nifty_50 = pd.read_csv(nifty50Path,parse_dates=["Date"])
        # nifty_50.head()
        col1.write(nifty_50)
        columns_info = {
    ...
    "Dividend Yield": "The amount of money a company pays shareholders (over the course of a year) for owning a share of its stock divided by its current stock price—displayed as a percentage."
        ...
        niftyAutoPath = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\datasets\NIFTY AUTO.csv"
        niftyBankPath = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\datasets\NIFTY BANK.csv"
        niftyFmcgPath = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\datasets\NIFTY FMCG.csv"
        niftyItPath = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\datasets\NIFTY IT.csv"
        niftyMetalPath = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\datasets\NIFTY METAL.csv"
        niftyPharmaPath = r"C:\Users\Asus\OneDrive\Desktop\Hackathon_project\Scale-91-Hackathon-FFI-2024\Streamlit_app\datasets\NIFTY PHARMA.csv"
        
        nifty_auto = pd.read_csv( niftyAutoPath ,parse_dates=["Date"])
        nifty_bank = pd.read_csv( niftyBankPath ,parse_dates=["Date"])
        nifty_fmcg = pd.read_csv( niftyFmcgPath ,parse_dates=["Date"])
        nifty_IT = pd.read_csv( niftyItPath ,parse_dates=["Date"])
        nifty_metal = pd.read_csv( niftyMetalPath ,parse_dates=["Date"])
        nifty_pharma = pd.read_csv( niftyPharmaPath ,parse_dates=["Date"])
    ...
    if (investment_range == 'Low' and duration == 'Mid-term' and 'Finance' in sectors_of_interest):
         with st.spinner('Loading...'):
            ...
            generate(pre_prompt,input,prompt_input)
        ...
        with st.spinner('Loading...'):
            ...
            pre_prompt = "You are a helpful assistant. You do not respond as 'User' or pretend to be 'User'. You only respond once as 'Assistant'."
            input_text = result_df[:1].to_string(index=False)  # Convert input DataFrame to string
            prompt_input = '...Generate a phrase showcasing the bank as a stock to invest for a 6-month range. Provide response to just this and nothing else. Assistant'
            generate(pre_prompt,input,prompt_input)
...
# Print the DataFrame
    ...
    # Footer
    st.markdown("---")
    st.markdown("Created by **Bhavya Pratap** & **Bhavya Madan**  ")
